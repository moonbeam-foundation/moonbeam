...
describeDevMoonbeam("Trace ethereum xcm #1", (context) => {
  ...
  before("should receive transact action with DescendOrigin", async function () {
    ...
    const transactions = (await context.web3.eth.getBlock("latest")).transactions;
  ...
  it.only("should trace ethereum xcm transactions with debug_traceTransaction", async function () {
