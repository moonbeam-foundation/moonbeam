...
pub struct MoonbeamFinality<B: Block, C> {
	pub backend: Arc<dyn fc_api::Backend<B> + Send + Sync>,
	pub client: Arc<C>,
	_phdata: PhantomData<B>,
}

impl<B: Block, C> MoonbeamFinality<B, C> {
	pub fn new(client: Arc<C>, backend: Arc<dyn fc_api::Backend<B> + Send + Sync>) -> Self {
		Self {
			backend,
			client,
...
fn is_block_finalized_inner<B: Block<Hash = H256>, C: HeaderBackend<B> + 'static>(
	backend: &(dyn fc_api::Backend<B> + Send + Sync),
	client: &C,
	raw_hash: H256,
) -> RpcResult<bool> {
	let substrate_hash =
