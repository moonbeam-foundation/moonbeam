...
- [ ] Validate migrations against live state. — **Partial (2026-06-19).**
  - *try-runtime (moonbase):* attempted (`on-runtime-upgrade --checks all live`), but stopped during the full state-scrape — no verdict. CLI + try-runtime moonbase wasm are built and ready for a retry, ideally via a saved snapshot (`create-snapshot` once → reuse).
  ...
  - **Authoritative migration validation still open:** try-runtime full-state (`--checks all`) snapshot, or the moonwall lazy-loading harness whose `lazyLoadingStateOverrides` pre-populate the storage-version keys.
