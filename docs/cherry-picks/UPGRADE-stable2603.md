...
- [x] Author migrations for any pallet whose `StorageVersion` advanced upstream (`writing-migrations` skill). — **Found one, fixed (2026-06-19).** An upstream-repo audit (diffing `STORAGE_VERSION` for every SDK pallet moonbeam uses, `origin/moonbeam-polkadot-stable2512` → `…2603`) found exactly one bump moonbeam must handle: **`cumulus-pallet-xcmp-queue` 5 → 6**. `MigrateV5ToV6` is a standalone `VersionedMigration` the pallet does *not* auto-run (upstream parachain runtimes — asset-hub, bridge-hub — wire it explicitly); moonbeam's migration lists were empty, so on-chain XcmpQueue (v5) wouldn't migrate and `OutboundXcmpStatus` would fail to decode against v6's new `flags` field → broken outbound XCMP. **Fixed:** added `cumulus_pallet_xcmp_queue::migration::v6::MigrateV5ToV6<Runtime>` to `UnreleasedSingleBlockMigrations` (commit `39d47d1d93`); compiles across default / try-runtime / runtime-benchmarks. No other SDK pallet moonbeam uses bumped its storage version. (Note: this is exactly the migration the lazy-loading run masked — it set XcmpQueue to v6 as a "new pallet" instead of running v5→v6.) **Test:** added `xcmp_queue_v5_to_v6_migration_is_wired_and_runs` (`runtime/common/src/tests/xcm.rs`, via `generate_common_xcm_tests!`) — sets XcmpQueue on-chain version to 5, runs the runtime's wired `SingleBlockMigrations`, asserts it ends at 6; passing on moonbase/moonbeam/moonriver (commit `4d578dfea3`). The upstream migration's own `pre`/`post` `try_state` is additionally covered by the deferred CI run.
- [x] Validate migrations against live state. — **Done (2026-06-19): partial local validation; full `try_state` deferred to CI.**
  - *try-runtime (moonbase):* attempted via both `on-runtime-upgrade --checks all live` and `create-snapshot`; both are gated by the full state scrape. In a timed run, `create-snapshot` was **still enumerating keys after a full 10 min** (phase 1 of 2, never completed) — the scrape is RPC-pagination-bound and impractical over the public endpoint (full snapshot ≈ tens of minutes to 1h+, with RPC-drop risk). A local archive-node DB or a dedicated endpoint would be needed to make this fast.
  - *Lazy-loading (moonbeam mainnet):* lazy-loading is moonbeam-only, so it can't target moonbase. Ran the node forking mainnet #16090736 with the new runtime as `--lazy-loading-runtime-override` (4500 vs live 4303); `on_runtime_upgrade` executed and block #16090738 + 14 more imported cleanly — **no panics/errors**. Confirms the new runtime loads and produces blocks on real mainnet state. **Caveat:** lazy state under-fetch logged many pallets as `🐥 new pallet … initializing storage version` (treated as fresh-init, not migrated), so per-pallet version-bump migrations were *not* fully exercised. Not a substitute for `try_state`.
  - **Decision (2026-06-19): local full `try_state` skipped — deferred to CI.** The public-RPC snapshot is impractical locally (above); CI runs try-runtime against proper infra. Residual risk is low: weights clean, the one required SDK migration (xcmp-queue v6) was found via the upstream-repo audit and wired, and the lazy-loading upgrade ran green on live mainnet state. To run locally in future: a faster archive endpoint / local DB, or the moonwall lazy-loading harness (its `lazyLoadingStateOverrides` pre-populate storage-version keys).
