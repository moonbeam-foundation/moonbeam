	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let old_multilocation = xcm::v2::MultiLocation {
				parents: 1,
				interior: xcm::v2::Junctions::X2(xcm::v2::Junction::Parachain(statemine_para_id), xcm::v2::Junction::GeneralIndex(1)),
			};
...
fn xcm_v2_to_v3_supported_assets_storage_migration_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let old_asset_types = vec![crate::migrations::OldAssetType::Xcm(old_multilocation.clone())];
