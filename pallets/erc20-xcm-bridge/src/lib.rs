...
pub mod pallet {
	...
	/// Represent an ERC-20 amount in XCM holding via a notional credit.
	///
	/// As of polkadot-sdk stable2603, `AssetsInHolding` tracks real
	/// `fungible::Credit` imbalances. ERC-20 balances live in the EVM and have no
	/// Substrate `fungible::Credit`, so we carry only the amount here; the actual
	/// token movement is performed by the corresponding `TransactAsset` leg.
	fn notional_holding(asset: &Asset, amount: U256) -> Result<AssetsInHolding, XcmError> {
	...
	impl<T: Config> xcm_executor::traits::TransactAsset for Pallet<T> {
		...
		fn deposit_asset(
			...
			if let Err(err) = Self::legacy_transactor_rejects_teleportable(&contract_address) {
				return Err((what, err));
			}
	...
	impl<T: Config> Erc20TeleportTransactor<T> {
		...
		fn ensure_trusted_peer(peer: &Location) -> Result<(), XcmError> {
	...
	impl<T: Config> xcm_executor::traits::TransactAsset for Erc20TeleportTransactor<T> {
		...
		fn deposit_asset(
			...
			// is gated by `IsTeleporter` + `can_check_in` + `mint_asset` (both bound to
			...
			let (contract, amount) = match Self::match_admitted(&asset) {
				Ok(matched) => matched,
				// `AssetNotHandled` maps to `AssetNotFound` so the asset-transactor tuple
				// falls through to the legacy reserve adapter for non-whitelisted ERC-20s.
				Err(err) => return Err((what, err.into())),
			};
		...
		fn withdraw_asset(
			...
			// fungible amount 0) via a notional credit.
			if amount.is_zero() {
				return notional_holding(what, amount);
			}
			...
			// Represent the locked erc20 amount in holding via a notional credit. The
			// real tokens already sit in `TeleportCheckingAccount`; when the executor
			// later drops this credit on `InitiateTeleport`, `NotionalImbalance` is a
			// no-op (unlike a real `fungible::Credit`, which would burn issuance).
			notional_holding(what, amount)
		}
		...
		// stable2603: the executor calls `mint_asset` when an asset is teleported in
		// (`ReceiveTeleportedAsset`) to put it into holding. ERC-20 has no real
		// `fungible::Credit`, so we represent it with a notional credit; the actual
		// unlock from `TeleportCheckingAccount` happens in `deposit_asset`.
		fn mint_asset(what: &Asset, _context: &XcmContext) -> Result<AssetsInHolding, XcmError> {
			let (_contract, amount) = Self::match_admitted(what).map_err(XcmError::from)?;
			notional_holding(what, amount)
		}
	}
