...
/// Build a single-asset `AssetsInHolding` for `deposit_asset` tests. ERC-20s are not arm
/// Substrate `fungible`, so the holding carries a notional credit (stable2603).
...
/// Extract the `XcmError` from the stable2603 deposit failure shape `(unspent, err)`.
fn deposit_err(result: Result<(), (AssetsInHolding, XcmError)>) -> Option<XcmError> {
	result.err().map(|(_, err)| err)
}
...
fn deposit_asset_promotes_and_saturating_subs_locked_supply() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Erc20TeleportTransactor::<Test>::deposit_asset(
			holding(&erc20_asset(contract.0, 100)),
			&beneficiary,
			None,
		));
		assert_eq!(
		...
		assert_ok!(Erc20TeleportTransactor::<Test>::deposit_asset(
			holding(&erc20_asset(contract.0, 1_000_000)),
			&beneficiary,
			None,
		));
...
fn zero_amount_legs_still_reject_non_whitelisted() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			deposit_err(Erc20TeleportTransactor::<Test>::deposit_asset(
				holding(&asset_zero),
				&user,
				None
			)),
...
fn lifecycle_end_to_end_purge_after_drain() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Erc20TeleportTransactor::<Test>::deposit_asset(
			holding(&erc20_asset(contract.0, 150)),
			&user,
			None,
		));
		assert_eq!(LockedSupply::<Test>::get(&contract), U256::from(0u128));
...
fn transactor_check_hooks_bind_to_trusted_location() {
	new_test_ext().execute_with(|| {
		...
		// Promote to `Active` before probing the untrusted-peer gate, so the ordering
		// guarantee ("trusted-peer check runs before status check") is covered for
		// both `Registered` (fresh add) and `Active`.
		TeleportableErc20s::<Test>::insert(&contract, TeleportableErc20Status::Active);
...
fn deregistered_split_transactor_withdraw_deposit_cannot_drain_checking_account() {
	new_test_ext().execute_with(|| {
		...
		test_with_xcm_holding_origins(|| {
			...
			assert_eq!(
				deposit_err(<Erc20XcmBridge as TransactAsset>::deposit_asset(
					holding(&asset),
					&attacker_loc,
					None
				)),
...
fn deposit_asset_admits_all_states_rejects_only_unknown() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			deposit_err(Erc20TeleportTransactor::<Test>::deposit_asset(
				holding(&erc20_asset(unknown.0, 100)),
				&user_loc([0x88; 20]),
				None,
			)),
			Some(XcmError::AssetNotFound),
		...
		assert_ok!(Erc20TeleportTransactor::<Test>::deposit_asset(
			holding(&erc20_asset(dereg.0, 200)),
			&user_loc([0x88; 20]),
			None,
		));
