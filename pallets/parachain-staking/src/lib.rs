...
pub mod pallet {
	...
	pub struct Nominator<AccountId, Balance> {
		...
		pub requests: PendingNominatorRequests<AccountId, Balance>,
		...
		> Nominator<AccountId, Balance>
	{
		pub fn new(collator: AccountId, amount: Balance) -> Self {
			Nominator {
				...
				total: amount,
				requests: PendingNominatorRequests::new(),
		...
		pub fn can_leave(&self, now: RoundIndex) -> Option<bool> {
		...
		/// Cancel a `bond_more` request for a nomination
		pub fn cancel_inc_nomination<T: Config>(&mut self, collator: AccountId) -> DispatchResult {
			self.requests
				.ensure_rm_request::<T>(NominatorAction::BondMore, collator)
		}
		...
		pub fn execute_inc_nomination<T: Config>(
			...
			ensure!(when <= &<Round<T>>::get().current, Error::<T>::NominatorDNE); // TODO relevant error
			let amt = amount.clone();
			self.requests.more_total -= amt;
			self.requests.request_per_collator.remove(&collator);
			// increase nomination
		...
		/// Cancel `bond_less` request to decrease nomination
		pub fn cancel_dec_nomination<T: Config>(&mut self, collator: AccountId) -> DispatchResult {
			self.requests
				.ensure_rm_request::<T>(NominatorAction::BondLess, collator)
		}
		/// Decrease nomination
			...
			BalanceOf<T>: Into<Balance>,
		{
			...
			if self.nominations.0.len() == 1usize {
				return Ok(None); // leave set of nominator instead
			}
		...
		pub fn execute_revoke<T: Config>(
			...
			let NominationChange {
				...
				.get(&collator)
				.ok_or(Error::<T>::NominatorDNE)?;
			ensure!(action == &NominatorAction::Revoke, Error::<T>::NominatorDNE); // TODO: relevant error
			ensure!(when <= &now, Error::<T>::NominatorDNE); // TODO relevant error
			let amt = amount.clone();
			self.requests.less_total -= amt;
			self.requests.request_per_collator.remove(&collator);
			self.rm_nomination(collator); // TODO: make infallible without ret type
			Ok(amt)
		}
	}
	...
	pub struct PendingNominatorRequests<AccountId, Balance> {
	...
	impl<A: Ord, B: Zero> Default for PendingNominatorRequests<A, B> {
		fn default() -> PendingNominatorRequests<A, B> {
			PendingNominatorRequests {
	...
	impl<
		...
		> PendingNominatorRequests<A, B>
	{
		/// New default (empty) pending requests
		pub fn new() -> PendingNominatorRequests<A, B> {
			PendingNominatorRequests::default()
		}
		...
		/// only works for the last 2, might as well just write one per each
		pub fn ensure_rm_request<T: Config>(
			&mut self,
			expect: NominatorAction,
			collator: A,
		) -> DispatchResult {
			if let Some(order) = self.request_per_collator.get(&collator) {
				ensure!(order.action == expect, Error::<T>::NominatorDNE); // TODO: relevant error
				match order.action {
				...
				// TODO: decide to keep this in or remove it after testing
				log::warn!("STATE IS INCONSISTENT");
			}
			Ok(())
		}
	}
	...
	pub trait Config: frame_system::Config {
		...
		/// Number of rounds that nominator bond {more, less} requests must wait before execution
		#[pallet::constant]
	...
	pub enum Event<T: Config> {
		...
		/// Nominator, Collator
		NominationRevocationCancelled(T::AccountId, T::AccountId),
		/// Nominator, Collator
		NominationIncreaseCancelled(T::AccountId, T::AccountId),
		/// Nominator, Collator
		NominationDecreaseCancelled(T::AccountId, T::AccountId),
	...
	type DelegatorState<T: Config> = StorageMap<
		...
		Twox64Concat,
		T::AccountId,
		Nominator<T::AccountId, BalanceOf<T>>, // TODO: rename to Delegator
		OptionQuery,
	>;
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(0)] // TODO: redo benchmarks
		...
		pub fn execute_leave_nominators(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
			...
			let allowed_to_leave = state
				.can_leave(<Round<T>>::get().current)
				.ok_or(Error::<T>::NominatorDNE)?; // Not leaving state
			ensure!(allowed_to_leave, Error::<T>::NominatorDNE); // cannot leave yet because not scheduled to leave yet (change error)
			...
			// TODO: NominatorExited would be more consistent naming for this event
			Self::deposit_event(Event::NominatorLeft(nominator, state.total));
			Ok(().into())
		}
		#[pallet::weight(0)] // TODO: benchmarking -> weights
		/// Cancel request to exit the set of delegator
		...
		pub fn schedule_revoke_nomination(
			...
			if let Some(remaining_future_total) =
				state.schedule_revoke::<T>(collator.clone(), when)?
			{
		...
		#[pallet::weight(<T as Config>::WeightInfo::revoke_nomination())]
		/// Cancel request to revoke an existing nomination
		pub fn cancel_revoke_nomination(
			origin: OriginFor<T>,
			collator: T::AccountId,
		) -> DispatchResultWithPostInfo {
			let nominator = ensure_signed(origin)?;
			...
			state.cancel_revoke::<T>(collator.clone())?;
			<DelegatorState<T>>::insert(&nominator, state);
			Self::deposit_event(Event::NominationRevocationCancelled(nominator, collator));
			Ok(().into())
		}
		#[pallet::weight(<T as Config>::WeightInfo::revoke_nomination())]
		/// Execute request to revoke an existing nomination
		pub fn execute_revoke_nomination(
			origin: OriginFor<T>,
			collator: T::AccountId,
		) -> DispatchResultWithPostInfo {
			let nominator = ensure_signed(origin)?;
			...
			let amount = state.execute_revoke::<T>(collator.clone(), <Round<T>>::get().current)?;
			Self::nominator_leaves_collator(nominator.clone(), collator.clone())?;
			<DelegatorState<T>>::insert(&nominator, state);
			Self::deposit_event(Event::NominationRevoked(nominator, collator, amount));
			Ok(().into())
		}
		#[pallet::weight(0)]
		/// Request to bond more for delegators with respect to a specific collator candidate
		pub fn schedule_increase_delegation(
			...
			let now = <Round<T>>::get().current;
			let when = now + T::NominatorBondDelay::get();
			state.schedule_inc_nomination::<T>(candidate.clone(), more, when)?;
			<DelegatorState<T>>::insert(&nominator, state);
		...
		#[pallet::weight(0)]
		/// Execute request to bond more for delegators wrt a specific collator candidate
		pub fn execute_increase_delegation(
			origin: OriginFor<T>,
			candidate: T::AccountId,
		) -> DispatchResultWithPostInfo {
			let nominator = ensure_signed(origin)?;
			...
			// execute bond more on delegator state
			let (more, new_nom_amt) = state.execute_inc_nomination::<T>(candidate.clone())?;
			let mut collator =
		...
		#[pallet::weight(0)]
		/// Cancel request to bond more for delegators with respect to a specific collator candidate
		pub fn cancel_increase_delegation(
			origin: OriginFor<T>,
			candidate: T::AccountId,
			more: BalanceOf<T>,
		) -> DispatchResultWithPostInfo {
			...
			state.cancel_inc_nomination::<T>(candidate.clone())?;
			<DelegatorState<T>>::insert(&nominator, state);
			Self::deposit_event(Event::NominationIncreaseCancelled(nominator, candidate));
			Ok(().into())
		}
		#[pallet::weight(<T as Config>::WeightInfo::nominator_bond_less())]
		/// Request bond less for nominators with respect to a specific nominator candidate
		/// - request can be executed `NominatorBondDelay` rounds after the round in which this
		/// extrinsic was called.
		pub fn schedule_decrease_delegation(
			...
			let now = <Round<T>>::get().current;
			let when = now + T::NominatorBondDelay::get();
			state.schedule_dec_nomination::<T>(candidate.clone(), less, when)?;
		...
		#[pallet::weight(<T as Config>::WeightInfo::nominator_bond_less())]
		/// Execute request to bond less for nominators wrt a specific collator candidate
		pub fn execute_decrease_delegation(
			origin: OriginFor<T>,
			candidate: T::AccountId,
		) -> DispatchResultWithPostInfo {
			let caller = ensure_signed(origin)?;
			...
			// execute bond less on delegator state
			let (less, new_nom_amt) = state.execute_dec_nomination::<T>(candidate.clone())?;
			ensure!(
				new_nom_amt >= T::MinNomination::get(),
				Error::<T>::NominationBelowMin
			);
			ensure!(
			...
			let mut collator =
				<CollatorState2<T>>::get(&candidate).ok_or(Error::<T>::CandidateDNE)?;
			T::Currency::unreserve(&caller, less);
			let before = collator.total_counted;
			let in_top = collator.dec_nominator(caller.clone(), less);
			let after = collator.total_counted;
			if collator.is_active() && (before != after) {
				Self::update_active(candidate.clone(), after);
			}
			...
			let new_total_staked = <Total<T>>::get().saturating_sub(less);
			<Total<T>>::put(new_total_staked);
			Self::deposit_event(Event::NominationDecreased(caller, candidate, less, in_top));
			Ok(().into())
		}
		#[pallet::weight(<T as Config>::WeightInfo::nominator_bond_less())]
		/// Bond less for nominators with respect to a specific nominator candidate
		pub fn cancel_decrease_delegation(
			origin: OriginFor<T>,
			candidate: T::AccountId,
		) -> DispatchResultWithPostInfo {
			let caller = ensure_signed(origin)?;
			...
			state.cancel_dec_nomination::<T>(candidate.clone())?;
			<DelegatorState<T>>::insert(&caller, state);
			Self::deposit_event(Event::NominationDecreaseCancelled(caller, candidate));
			Ok(().into())
		}
	}
				...
				.filter_map(|(nominator, maybe_collator, when)| {
					if when > now {
						...
						if let Some(collator) = maybe_collator {
							...
							if let Some(mut state) = <DelegatorState<T>>::get(&nominator) {
							...
							if let Some(state) = <DelegatorState<T>>::get(&nominator) {
								for bond in state.nominations.0 {
