...
mod benchmarks {
	...
	#[benchmark]
	fn verify_entry(x: Linear<100, 1000>) -> Result<(), BenchmarkError> {
		...
		let x = (x / 100) * 100;
		let (state_root, mocked_proof) = mocked_proofs.get(&x).expect("Not Found").clone();
	...
	fn p256_verify() -> Result<(), BenchmarkError> {
		...
		{
			let _ =
				p256verify::<T>(input).expect("Should verify the signature without any errors.");
		}
