...
mod benchmarks {
	...
	fn prepare_for_open_bridge<T: Config<I>, I: 'static>(
		...
		let (origin, initial_balance) = T::open_bridge_origin().ok_or(BenchmarkError::Override(
	...
	fn open_bridge() -> Result<(), BenchmarkError> {
		...
		let bridge_destination_universal_location: Box<VersionedInteriorLocation> = Box::new(
			locations
				.bridge_destination_universal_location()
				.clone()
				.into(),
