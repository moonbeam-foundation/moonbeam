...
impl<T: Config> OnRuntimeUpgrade for TransactSignedWeightAndFeePerSecond<T> {
	fn on_runtime_upgrade() -> Weight {
		...
		for (location, info) in stored_data {
			...
			DestinationFeePerSecond::<T>::insert(&location, info.fee_per_second);
		}
	...
	fn pre_upgrade() -> Result<(), &'static str> {
		...
		// As long as there are some mappings stored, one representative key maps to the
		// same value after the migration.
		// There are no entries in the new DestinationFeePerSecond beforehand

		// Assert DestinationFeePerSecond storage is empty
		assert!(DestinationFeePerSecond::<T>::iter().next().is_none());
