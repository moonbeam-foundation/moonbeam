...
use crate::mock::{
	events, roll_to, roll_to_round_begin, set_points, Call, ExtBuilder, Origin, PCall,
	ParachainStaking, Precompiles, PrecompilesValue, Runtime,
};
...
fn go_online_works() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(ParachainStaking::go_offline(Origin::signed(Alice.into())));
