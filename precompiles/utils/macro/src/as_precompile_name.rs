...
pub fn main(_: TokenStream, input: TokenStream) -> TokenStream {
	...
	if let Type::Tuple(ref type_tuple) = *ty {
		let variants: Vec<(Ident, u64)> = type_tuple
			.elems
			.iter()
			.filter_map(as_precompile_name_variant)
			.collect();
...
fn as_precompile_name_variant(type_: &Type) -> Option<(Ident, u64)> {
			...
			if let syn::PathArguments::AngleBracketed(generics) = &path_segment.arguments {
				...
				) = (iter.next(), iter.next())
				{
					if let (Some(path_segment_1), Some(path_segment_2)) = (
						...
						if let syn::PathArguments::AngleBracketed(generics_) =
							&path_segment_1.arguments
						{
							if let Some(GenericArgument::Const(Expr::Lit(lit))) =
								generics_.args.first()
							{
								if let Lit::Int(int) = &lit.lit {
									if let Ok(precompile_id) = int.base10_parse() {
										if &path_segment_2.ident.to_string()
											== "CollectivePrecompile"
										{
											if let syn::PathArguments::AngleBracketed(generics_) =
												&path_segment_2.arguments
											{
												...
												))) = generics_.args.last()
												{
													if let Some(instance_type) =
														instance_type_path.path.segments.last()
													{
														return Some((
															instance_type.ident.clone(),
															precompile_id,
														));
													}
												}
