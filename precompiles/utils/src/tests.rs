...
fn write_vec_bytes() {
	...
	assert_eq!(reader.read::<U256>().expect("read offset"), 0xc0.into());
...
fn read_complex_solidity_function() {
	...
	let data = hex!(
		""
	);

	let (mut reader, selector) =
		EvmDataReader::new_with_selector::<Action>(&data).expect("to read selector");

	assert_eq!(selector, Action::Transfer);
	assert_eq!(
		reader.read::<Address>().unwrap(),
		H160::repeat_byte(0xdd).into()
	);
	assert_eq!(reader.read::<U256>().unwrap(), 500u32.into());
	assert_eq!(
		reader.read::<MultiLocation>().unwrap(),
		MultiLocation {
			parents: 2,
			interior: vec![Bytes::from(&[1u8; 33][..])],
		}
	);
