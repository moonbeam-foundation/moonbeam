...
use evm::{ExitSucceed, Handler, Opcode, gasometer};
...
impl State {
    ...
    pub fn after_step<'config, S: StackState<'config>, H: Hook>(
        ...
        let context = self.context_stack
            .last_mut()
    ...
    pub fn after_loop<'config, S: StackState<'config>, H: Hook>(
        ...
        // now.
        if let Some(before_step) = context
            .before_step
            .take()
        {
            self.end_step(executor, before_step);
    ...
    fn end_step<'config, S: StackState<'config>, H: Hook>(
        ...
        if let Some(key) = storage_key {
            let _ = context.storage_cache.insert(
                key, 
                executor.storage(context.address, key)
            );
        }
