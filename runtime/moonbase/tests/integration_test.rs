...
fn reward_block_authors() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Balances::free_balance(AccountId::from(ALICE)), 2_000 * UNIT,);
			assert_eq!(Balances::free_balance(AccountId::from(BOB)), 1_000 * UNIT,);
			run_to_block(1200, Some(NimbusId::from_slice(&ALICE_NIMBUS).unwrap()));
			// rewards minted and distributed
			assert_eq!(
				Balances::free_balance(AccountId::from(ALICE)),
				2113666666584000000000,
			);
			assert_eq!(
				Balances::free_balance(AccountId::from(BOB)),
				1041333333292000000000,
			);
		});
}
...
fn reward_block_authors_with_parachain_bond_reserved() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Balances::free_balance(AccountId::from(ALICE)),
				2082693333281650000000,
			);
			assert_eq!(
				Balances::free_balance(AccountId::from(BOB)),
				1025841666640825000000,
			);
