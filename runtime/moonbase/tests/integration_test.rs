...
fn candidate_count_via_precompile() {
	ExtBuilder::default()
		.with_balances(vec![
			(AccountId::from(ALICE), 1_100 * UNIT),
			(AccountId::from(BOB), 1_100 * UNIT),
			(AccountId::from(CHARLIE), 1_100 * UNIT),
			(AccountId::from(DAVE), 1_100 * UNIT),
		])
		.with_collators(vec![
			(AccountId::from(ALICE), 1_000 * UNIT),
			(AccountId::from(BOB), 1_000 * UNIT),
			(AccountId::from(CHARLIE), 1_000 * UNIT),
			(AccountId::from(DAVE), 1_000 * UNIT),
		])
		.build()
		.execute_with(|| {
...
fn collator_nomination_count_via_precompile() {
	ExtBuilder::default()
		...
		.with_collators(vec![(AccountId::from(ALICE), 1_000 * UNIT)])
		.with_nominations(vec![
			(AccountId::from(BOB), AccountId::from(ALICE), 100 * UNIT),
			(AccountId::from(CHARLIE), AccountId::from(ALICE), 100 * UNIT),
			(AccountId::from(DAVE), AccountId::from(ALICE), 100 * UNIT),
		])
		.build()
		.execute_with(|| {
			let staking_precompile_address = H160::from_low_u64_be(2048);
...
fn nominator_nomination_count_via_precompile() {
	ExtBuilder::default()
		...
		.with_nominations(vec![
			(AccountId::from(DAVE), AccountId::from(ALICE), 100 * UNIT),
			(AccountId::from(DAVE), AccountId::from(BOB), 100 * UNIT),
			(AccountId::from(DAVE), AccountId::from(CHARLIE), 100 * UNIT),
		])
		.build()
		.execute_with(|| {
			let staking_precompile_address = H160::from_low_u64_be(2048);
