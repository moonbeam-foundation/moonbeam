...
fn foreign_asset_transactor_deposits_registered_asset() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let balance_before = moonbase_runtime::EvmForeignAssets::balance(1, bob_account())
				.unwrap_or(U256::zero());
			...
			let balance_after = moonbase_runtime::EvmForeignAssets::balance(1, bob_account())
...
fn transactor_handles_erc20_bridge_asset() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			use moonbase_runtime::xcm_config::AssetTransactors;
			use moonbase_runtime::Erc20XcmBridge;
