...
//! `asset-hub-westend-runtime` (para 1000) and the real `moonbase-runtime`
...
#[test]
fn relay_funds_both_asset_hub_and_moonbase() {
	...
	fund_moonbase_alith_with_dot(ONE_DOT * 10);
	...
	let moonbase_balance = moonbase_execute_with(|| {
	...
	assert!(
		moonbase_balance > U256::zero(),
		"Moonbase should have DOT (got {moonbase_balance})"
	);
}
...
#[test]
fn transfer_trust_backed_asset_from_asset_hub_to_moonbase() {
	setup_asset_hub_and_moonbase();
	...
	let alith_usdt = moonbase_execute_with(|| {
		moonbase_runtime::EvmForeignAssets::balance(
			USDT_FOREIGN_ID,
			moonbase_runtime::AccountId::from(ALITH),
		)
