...
//! XcmTransactor tests using the **real** Moonbase runtime against Westend relay.
//!
//! Covers:
...
fn setup_transactor() {
	...
	moonbase_execute_with(|| {
		...
		assert_ok!(moonbase_runtime::XcmTransactor::set_transact_info(
	...
	// Fund Moonbase's sovereign on relay so it can pay fees for UMP transacts.
...
fn relay_remark_call() -> Vec<u8> {
	westend_runtime::RuntimeCall::System(
		frame_system::Call::<westend_runtime::Runtime>::remark_with_event {
			remark: b"hello from Moonbase".to_vec(),
		},
	)
	.encode()
}
...
/// Send DOT from relay to Moonbase ALITH.
...
fn transact_through_sovereign_to_relay() {
	setup_transactor();
	fund_moonbase_alith_with_dot(ONE_DOT * 1000);
	...
	moonbase_execute_with(|| {
		assert_ok!(moonbase_runtime::XcmTransactor::transact_through_sovereign(
			moonbase_runtime::RuntimeOrigin::root(),
			Box::new(xcm::VersionedLocation::from(Location::parent())),
			Some(moonbase_runtime::AccountId::from(ALITH)),
			CurrencyPayment {
...
fn hrmp_init_accept_close_via_xcm_transactor() {
	...
	// Step 1: Moonbase requests to open channel to sibling.
	moonbase_execute_with(|| {
		assert_ok!(moonbase_runtime::XcmTransactor::hrmp_manage(
			moonbase_runtime::RuntimeOrigin::root(),
...
fn hrmp_close_via_xcm_transactor() {
	...
	// Close the channel from Moonbase side via XcmTransactor.
	moonbase_execute_with(|| {
		assert_ok!(moonbase_runtime::XcmTransactor::hrmp_manage(
			moonbase_runtime::RuntimeOrigin::root(),
...
fn transact_through_signed_to_relay() {
	...
	let derived_account = relay_derived_account(MOONBASE_PARA_ID, ALITH);
	...
	moonbase_execute_with(|| {
		assert_ok!(moonbase_runtime::XcmTransactor::transact_through_signed(
...
fn setup_derivative() {
	...
	moonbase_execute_with(|| {
		assert_ok!(moonbase_runtime::XcmTransactor::register(
			moonbase_runtime::RuntimeOrigin::root(),
	...
	// The derivative is computed from the sovereign account of Moonbase parachain.
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		let sovereign = WestendRelay::<PolkadotMoonbeamNet>::sovereign_account_id_of(
			Location::new(0, [Parachain(MOONBASE_PARA_ID)]),
		);
...
/// Opens HRMP channels between Moonbase and Sibling, registers DOT on both,
/// and funds the derived account on the sibling.
fn setup_para_to_para_signed() -> moonbase_runtime::AccountId {
	init_network();

	// Register DOT + relay indices on Moonbase.
	moonbase_execute_with(|| {
		register_dot_asset(DOT_ASSET_ID);
		set_westend_relay_indices();
	});
	...
	// Compute the derived account on the sibling for ALITH's signed origin from Moonbase.
	// After DescendOrigin(AccountKey20(ALITH)), the sibling sees origin
	// Location::new(1, [Parachain(2004), AccountKey20(ALITH)]).
	let derived_on_sibling: moonbase_runtime::AccountId = sibling_execute_with(|| {
		<moonbase_runtime::xcm_config::LocationToAccountId as ConvertLocation<
			moonbase_runtime::AccountId,
		...
		.expect("Should derive sibling account for Moonbase ALITH")
	});
...
/// Encode a `system::remark_with_event` call for the sibling (Moonbase runtime).
fn sibling_remark_call() -> Vec<u8> {
	moonbase_runtime::RuntimeCall::System(
		frame_system::Call::<moonbase_runtime::Runtime>::remark_with_event {
			remark: b"hello from Moonbase to sibling".to_vec(),
		},
	)
	.encode()
}
...
fn assert_sibling_remark_executed() {
	sibling_execute_with(|| {
		...
		let has_remark = events.iter().any(|e| {
			matches!(
				&e.event,
				moonbase_runtime::RuntimeEvent::System(frame_system::Event::Remarked { .. })
			)
		});
		assert!(
			has_remark,
...
fn setup_para_to_para_ethereum() -> moonbase_runtime::AccountId {
	let derived_on_sibling = setup_para_to_para_signed();
	...
	sibling_execute_with(|| {
		<moonbase_runtime::Balances as Mutate<_>>::mint_into(
			&derived_on_sibling,
			moonbase_runtime::currency::UNIT * 10,
		)
...
fn transact_through_signed_para_to_para_ethereum_proxy_succeeds() {
	...
	sibling_execute_with(|| {
		assert_ok!(moonbase_runtime::Proxy::add_proxy(
			moonbase_runtime::RuntimeOrigin::signed(moonbase_runtime::AccountId::from(ALITH)),
			derived,
			moonbase_runtime::ProxyType::Any,
			0,
		));
	});
