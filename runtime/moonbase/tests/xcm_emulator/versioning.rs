...
fn xcm_version_discovery_with_relay() {
	...
	// Verify the relay can version-wrap an XCM destined for Moonbase.
	...
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		...
		assert!(
			fees.is_ok(),
			"Relay should resolve XCM version for Moonbase destination"
		);
	});

	// Verify Moonbase can version-wrap an XCM destined for the relay.
	...
	moonbase_execute_with(|| {
		...
		assert!(
			fees.is_ok(),
			"Moonbase should resolve XCM version for relay destination"
		);
	});
}
...
fn xcm_version_discovery_with_sibling() {
	...
	// Verify sibling can version-wrap an XCM destined for Moonbase.
	...
	sibling_execute_with(|| {
		...
		assert!(
			fees.is_ok(),
			"Sibling should resolve XCM version for Moonbase destination"
		);
	});

	// Verify Moonbase can version-wrap an XCM destined for the sibling.
	moonbase_execute_with(|| {
		use xcm_runtime_apis::fees::runtime_decl_for_xcm_payment_api::XcmPaymentApiV1;
		let fees = moonbase_runtime::Runtime::query_delivery_fees(
		...
		assert!(
			fees.is_ok(),
			"Moonbase should resolve XCM version for sibling destination"
		);
	});
}
