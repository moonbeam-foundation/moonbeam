// Copyright 2019-2025 Moonbase Foundation.
// This file is part of Moonbase.

// Moonbase is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.

// Moonbase is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.

// You should have received a copy of the GNU General Public License
// along with Moonbase.  If not, see <http://www.gnu.org/licenses/>.

//! Transfer tests using xcm-emulator with the **real** Moonbase runtime.
//!
...
/// Full network init: register DOT on Moonbase, configure weight trader.
fn setup_relay_to_moonbase() {
	init_network();
...
// ===========================================================================
// Transfer: Relay → Moonbase (DMP)
// ===========================================================================

#[test]
fn transfer_dot_from_relay_to_moonbase() {
	...
	moonbase_execute_with(|| {
		let beneficiary = moonbase_runtime::AccountId::from(beneficiary_key);
		let balance = moonbase_runtime::EvmForeignAssets::balance(DOT_ASSET_ID, beneficiary)
			.expect("balance query should succeed");
		assert!(
			balance > U256::zero(),
			"Beneficiary should have DOT on Moonbase, got {balance}"
		);
	});
}

// ===========================================================================
// Transfer: Moonbase → Relay (UMP)
// ===========================================================================

#[test]
fn transfer_dot_from_moonbase_to_relay() {
	setup_relay_to_moonbase();

	// First: send DOT from relay to Moonbase so ALITH has some DOT.
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		assert_ok!(
			westend_runtime::XcmPallet::transfer_assets_using_type_and_then(
	...
	let alith_dot_before = moonbase_execute_with(|| {
	...
	// Now send DOT back from Moonbase to relay via PolkadotXcm.
...
fn error_when_not_paying_enough_fees() {
	...
	// Send a tiny amount (1 unit) from relay — should fail to pay Moonbase execution fees.
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		assert_ok!(
			westend_runtime::XcmPallet::transfer_assets_using_type_and_then(
				westend_runtime::RuntimeOrigin::signed(RELAY_ALICE),
...
fn fees_collected_by_treasury() {
	...
	let treasury_dot_before = moonbase_execute_with(|| {
		let treasury = moonbase_runtime::Treasury::account_id();
		moonbase_runtime::EvmForeignAssets::balance(DOT_ASSET_ID, treasury).unwrap_or(U256::zero())
	});

	// Send DOT from relay to Moonbase (fees will be charged).
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		assert_ok!(
			westend_runtime::XcmPallet::transfer_assets_using_type_and_then(
				westend_runtime::RuntimeOrigin::signed(RELAY_ALICE),
	...
	moonbase_execute_with(|| {
		...
		let treasury_dot_after =
			moonbase_runtime::EvmForeignAssets::balance(DOT_ASSET_ID, treasury)
				.unwrap_or(U256::zero());
		assert!(
		...
		let beneficiary_balance = moonbase_runtime::EvmForeignAssets::balance(
			DOT_ASSET_ID,
...
fn receive_asset_for_non_existent_account() {
	...
	moonbase_execute_with(|| {
		let balance = moonbase_runtime::EvmForeignAssets::balance(
			DOT_ASSET_ID,
			moonbase_runtime::AccountId::from(fresh_account),
		)
		.unwrap();
		assert!(
			balance > U256::zero(),
...
#[test]
fn transfer_dot_from_moonbase_to_sibling() {
	setup_with_sibling();

	// First fund Moonbase ALITH with DOT from relay.
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		assert_ok!(
			westend_runtime::XcmPallet::transfer_assets_using_type_and_then(
				westend_runtime::RuntimeOrigin::signed(RELAY_ALICE),
	...
	// Verify ALITH got DOT on Moonbase.
	let alith_dot = moonbase_execute_with(|| {
		moonbase_runtime::EvmForeignAssets::balance(
			DOT_ASSET_ID,
			moonbase_runtime::AccountId::from(ALITH),
		)
	...
	// Now send DOT from Moonbase to Sibling via reserve transfer through relay.
...
fn evm_account_receives_foreign_asset() {
	...
	moonbase_execute_with(|| {
		...
		let glmr = <moonbase_runtime::Balances as Inspect<_>>::balance(
			&moonbase_runtime::AccountId::from(ALITH),
		);
		assert!(glmr > 0, "ALITH should still have GLMR");

		let dot = moonbase_runtime::EvmForeignAssets::balance(
			DOT_ASSET_ID,
			moonbase_runtime::AccountId::from(ALITH),
		)
		.unwrap();
...
fn foreign_assets_survive_native_balance_drain() {
	...
	moonbase_execute_with(|| {
		<moonbase_runtime::Balances as Mutate<_>>::mint_into(
			&moonbase_runtime::AccountId::from(test_account),
			moonbase_runtime::currency::UNIT,
		)
	...
	moonbase_execute_with(|| {
		let balance = <moonbase_runtime::Balances as Inspect<_>>::balance(
			&moonbase_runtime::AccountId::from(test_account),
		);
		let _ = <moonbase_runtime::Balances as Mutate<_>>::burn_from(
			&moonbase_runtime::AccountId::from(test_account),
			balance,
...
const UNIT_ASSET_ID: u128 = 2;

/// Register Moonbase's native GLMR as a foreign asset on the sibling and
...
/// Setup for GLMR para→para transfers: open HRMP, register DOT on Moonbase,
/// register GLMR on sibling.
fn setup_unit_para_to_para() {
	setup_with_sibling();
	register_unit_on_sibling();
}

/// Transfer GLMR from Moonbase to Sibling (reserve-backed).
#[test]
fn transfer_unit_from_moonbase_to_sibling() {
...
/// Roundtrip: GLMR from Moonbase → Sibling → back to Moonbase.
#[test]
fn transfer_unit_roundtrip_moonbase_sibling() {
	setup_unit_para_to_para();
	...
	// Step 1: Send GLMR from Moonbase to Sibling (BALTATHAR).
	moonbase_execute_with(|| {
		assert_ok!(moonbase_runtime::PolkadotXcm::transfer_assets(
			moonbase_runtime::RuntimeOrigin::signed(moonbase_runtime::AccountId::from(ALITH)),
			Box::new(xcm::VersionedLocation::from(Location::new(
...
#[test]
fn transfer_unit_to_sibling_with_trader_fees() {
	setup_unit_para_to_para();

	let amount = moonbase_runtime::currency::UNIT * 100; // 100 GLMR
	...
	sibling_execute_with(|| {
		let received = moonbase_runtime::EvmForeignAssets::balance(
			UNIT_ASSET_ID,
			moonbase_runtime::AccountId::from(BALTATHAR),
		)
		.unwrap();
...
fn fund_moonbase_alith_with_dot(amount: u128) {
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		let beneficiary = Location::new(
			0,
			[AccountKey20 {
...
/// Send DOT from Moonbase to a sibling using `RemoteReserve` through the
...
fn transfer_dot_to_sibling_via_remote_reserve() {
	...
	fund_moonbase_alith_with_dot(send_amount);
	...
	let alith_dot_after = moonbase_execute_with(|| {
		moonbase_runtime::EvmForeignAssets::balance(
			DOT_ASSET_ID,
			moonbase_runtime::AccountId::from(ALITH),
		)
...
/// Roundtrip: DOT from Moonbase → Sibling → back to Moonbase, both legs
...
fn transfer_dot_roundtrip_via_remote_reserve() {
	...
	sibling_execute_with(|| {
		assert_ok!(
			moonbase_runtime::PolkadotXcm::transfer_assets_using_type_and_then(
				moonbase_runtime::RuntimeOrigin::signed(moonbase_runtime::AccountId::from(
					BALTATHAR,
				)),
				Box::new(xcm::VersionedLocation::from(Location::new(
					1,
	...
	let alith_dot_final = moonbase_execute_with(|| {
		moonbase_runtime::EvmForeignAssets::balance(
			DOT_ASSET_ID,
			moonbase_runtime::AccountId::from(ALITH),
		)
...
#[test]
fn transfer_unit_self_reserve_to_sibling() {
	setup_with_sibling();
	register_unit_on_sibling();

	let glmr_amount = moonbase_runtime::currency::UNIT;
...
/// Receive a sibling-native foreign asset on Moonbase.
/// A sibling sends its own native token (another Moonbase instance's GLMR)
/// to Moonbase, which receives it as an EVM foreign asset.
#[test]
fn receive_sibling_native_asset() {
	setup_with_sibling();

	// On Moonbase, register the sibling's GLMR (PalletInstance(3) on para 2005)
	...
	assert!(
		bal > U256::zero(),
		"BALTATHAR should have sibling GLMR on Moonbase (got {bal})"
	);
}
