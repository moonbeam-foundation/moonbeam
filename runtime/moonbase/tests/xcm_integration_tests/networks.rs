...
use crate::chains::{
	...
	moonbase::{moonbase_ext, MOONBASE_PARA_ID},
...
thread_local! {
	...
	pub static MOONBASE_EXT: RefCell<TestExternalities> = RefCell::new(moonbase_ext());
...
pub fn reset_networks() {
	...
	MOONBASE_EXT.with(|ext| *ext.borrow_mut() = moonbase_ext());
...
pub fn moonbase_execute_with<R>(f: impl FnOnce() -> R) -> R {
	MOONBASE_EXT.with(|ext| ext.borrow_mut().execute_with(f))
}
...
pub fn process_dmp_messages() {
	while let Some((para_id, msg)) = DMP_QUEUE.with(|q| q.borrow_mut().pop_front()) {
		...
		match para_id_u32 {
			id if id == MOONBASE_PARA_ID => {
				moonbase_execute_with(|| {
					// Decode the versioned XCM and execute directly
					if let Ok(versioned) =
						xcm::VersionedXcm::<moonbase_runtime::RuntimeCall>::decode(&mut &msg[..])
					{
						if let Ok(xcm_msg) = versioned.try_into() {
							let origin = Location::parent();
							let mut hash = sp_io::hashing::blake2_256(&msg);
							let _ = XcmExecutor::<moonbase_runtime::xcm_config::XcmExecutorConfig>::prepare_and_execute(
                                origin,
                                xcm_msg,
                                &mut hash,
                                Weight::MAX,
...
pub fn moonbase_execute_and_dispatch<R>(f: impl FnOnce() -> R) -> R {
	let result = moonbase_execute_with(f);
	dispatch_xcm_buses();
	result
}
...
pub fn moonbase_sovereign_account() -> sp_runtime::AccountId32 {
	parachain_sovereign_account(MOONBASE_PARA_ID)
}
...
pub mod para_ids {
	pub const MOONBASE: u32 = super::MOONBASE_PARA_ID;
