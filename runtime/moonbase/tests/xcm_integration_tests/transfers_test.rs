...
//! - Transfer UNIT from Moonbeam to relay
...
fn transfer_dot_from_relay_is_configured_correctly() {
	...
	moonbase_execute_with(|| {
		...
		use moonbase_runtime::xcm_config::RelayLocation;
		assert_eq!(
			RelayLocation::get(),
			Location::parent(),
...
fn transfer_from_asset_hub_configured() {
	moonbase_execute_with(|| {
		use moonbase_runtime::xcm_config::AssetHubLocation;
		...
		assert_eq!(
			...
			"Asset Hub should be parachain 1001"
		);
...
fn teleport_is_not_supported() {
	...
	moonbase_execute_with(|| {
		...
		use moonbase_runtime::xcm_config::XcmExecutorConfig;
...
fn reserve_transfer_supported_for_self_reserve() {
	moonbase_execute_with(|| {
		...
		use moonbase_runtime::xcm_config::SelfReserve;
		assert_eq!(
			SelfReserve::get(),
			self_reserve,
...
fn relay_can_send_dmp_to_moonbeam() {
	...
	let moonbeam_sovereign = moonbase_sovereign_account();
	...
	relay_execute_with(|| {
		...
		let message: Xcm<RuntimeCall> = Xcm(vec![
			...
			DepositReserveAsset {
				assets: Wild(All),
				dest: Location::new(0, [Parachain(para_ids::MOONBASE)]),
				xcm: Xcm(vec![
					BuyExecution {
...
fn cross_chain_infrastructure_is_functional() {
	...
	let moonbeam_sov = moonbase_sovereign_account();
