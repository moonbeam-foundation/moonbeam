...
fn set_fee_per_second_for_location(location: Location, fee_per_second: u128) -> Result<(), ()> {
	...
	if pallet_xcm_weight_trader::SupportedAssets::<moonbeam_runtime::Runtime>::contains_key(&location) {
		let enabled = pallet_xcm_weight_trader::SupportedAssets::<moonbeam_runtime::Runtime>::get(&location)
			.ok_or(())?
			.0;
