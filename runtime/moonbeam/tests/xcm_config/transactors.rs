...
//! Moonbeam uses a tuple of transactors:
//! - LocalAssetTransactor: Handles native token (GLMR) using pallet_balances
...
fn local_transactor_fails_withdraw_insufficient_balance() {
	ExtBuilder::default()
		.with_balances(vec![(alice_account(), ONE_GLMR)]) // Only 1 GLMR
		.build()
		.execute_with(|| {
			use moonbeam_runtime::xcm_config::AssetTransactors;
			...
			let asset = Asset {
				id: AssetId(native_asset_location()),
				fun: Fungible(ONE_GLMR * 100), // Try to withdraw 100 GLMR
			};
			let source = Location::new(
				0,
				[AccountKey20 {
...
fn foreign_asset_transactor_deposits_registered_asset() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let balance_before = moonbeam_runtime::EvmForeignAssets::balance(1, bob_account())
				.unwrap_or(U256::zero());
			...
			let balance_after = moonbeam_runtime::EvmForeignAssets::balance(1, bob_account())
...
fn transactor_handles_relay_sovereign_account() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			use moonbeam_runtime::xcm_config::{AssetTransactors, LocationToAccountId};
			...
			let _ = Balances::deposit_creating(&sovereign_account, ONE_GLMR * 10);
