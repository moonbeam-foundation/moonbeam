...
fn xcm_version_discovery_with_relay() {
	...
	// Verify the relay can version-wrap an XCM destined for Moonbeam.
	...
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		...
		assert!(
			fees.is_ok(),
			"Relay should resolve XCM version for Moonbeam destination"
		);
	});
	...
	moonbeam_execute_with(|| {
		...
		assert!(
			fees.is_ok(),
			"Moonbeam should resolve XCM version for relay destination"
		);
	});
}
...
fn xcm_version_discovery_with_sibling() {
	...
	// Verify sibling can version-wrap an XCM destined for Moonbeam.
	...
	sibling_execute_with(|| {
		...
		assert!(
			fees.is_ok(),
			"Sibling should resolve XCM version for Moonbeam destination"
		);
	});
	...
	moonbeam_execute_with(|| {
		...
		assert!(
			fees.is_ok(),
			"Moonbeam should resolve XCM version for sibling destination"
		);
	});
}
