...
//! - Transfer GLMR from Moonbeam to relay
...
use crate::common::*;
use crate::networks::*;
use moonbeam_runtime::{currency::GLMR, xcm_config::LocationToAccountId, AccountId, Balances};
...
fn transfer_dot_from_relay_is_configured_correctly() {
	...
	moonbeam_execute_with(|| {
		...
		use moonbeam_runtime::xcm_config::RelayLocation;
		assert_eq!(
			RelayLocation::get(),
			Location::parent(),
...
fn teleport_is_not_supported() {
	...
	moonbeam_execute_with(|| {
		...
		use moonbeam_runtime::xcm_config::XcmExecutorConfig;
...
fn reserve_transfer_supported_for_self_reserve() {
	moonbeam_execute_with(|| {
		...
		use moonbeam_runtime::xcm_config::SelfReserve;
		assert_eq!(
			SelfReserve::get(),
			self_reserve,
...
fn relay_can_send_dmp_to_moonbeam() {
	...
	let moonbeam_sovereign = moonbeam_sovereign_account();
	...
	relay_execute_with(|| {
		...
		let message: Xcm<RuntimeCall> = Xcm(vec![
			...
			DepositReserveAsset {
				assets: Wild(All),
				dest: Location::new(0, [Parachain(para_ids::MOONBEAM)]),
				xcm: Xcm(vec![
					BuyExecution {
...
fn cross_chain_infrastructure_is_functional() {
	...
	let moonbeam_sov = moonbeam_sovereign_account();
