...
fn setup_para_to_para_signed() -> moonbeam_runtime::AccountId {
	...
	sibling_execute_with(|| {
		let balance =
			moonbeam_runtime::EvmForeignAssets::balance(DOT_ASSET_ID, derived_on_sibling).unwrap();
		assert!(
...
fn ethereum_xcm_transfer_call(recipient: sp_core::H160, value: u128) -> Vec<u8> {
	...
	moonbeam_runtime::RuntimeCall::EthereumXcm(pallet_ethereum_xcm::Call::<
		moonbeam_runtime::Runtime,
	>::transact {
...
fn transact_through_signed_para_to_para_ethereum_no_proxy_fails() {
	...
	let proxy_call = moonbeam_runtime::RuntimeCall::EthereumXcm(pallet_ethereum_xcm::Call::<
		moonbeam_runtime::Runtime,
