...
//!
//! Wires a Westend relay chain, the real Moonriver runtime (para 2004),
//! and a sibling Moonriver instance (para 2005) into a single test network.
...
/// `BlockProducer` for Moonriver's Nimbus-based consensus.
///
/// xcm-emulator defaults to an Aura-backed producer; Moonriver does not use
/// pallet-aura, so we plug in a Nimbus pre-runtime digest with a 6s block
/// interval (matching `moonriver_runtime::MILLISECS_PER_BLOCK`).
pub struct MoonriverBlockProducer;

impl BlockProducer for MoonriverBlockProducer {
	fn slot_duration() -> u64 {
		moonriver_runtime::MILLISECS_PER_BLOCK
	}
...
// ---------------------------------------------------------------------------
// Moonriver parachain declaration (para 2004)
// ---------------------------------------------------------------------------
decl_test_parachains! {
	pub struct MoonriverPara {
		...
		runtime = moonriver_runtime,
		core = {
			XcmpMessageHandler: moonriver_runtime::XcmpQueue,
			LocationToAccountId: moonriver_runtime::xcm_config::LocationToAccountId,
			ParachainInfo: moonriver_runtime::ParachainInfo,
			MessageOrigin: cumulus_primitives_core::AggregateMessageOrigin,
			BlockProducer: crate::network::MoonriverBlockProducer,
		},
		pallets = {
			PolkadotXcm: moonriver_runtime::PolkadotXcm,
			Balances: moonriver_runtime::Balances,
			EvmForeignAssets: moonriver_runtime::EvmForeignAssets,
			XcmWeightTrader: moonriver_runtime::XcmWeightTrader,
			XcmTransactor: moonriver_runtime::XcmTransactor,
			Treasury: moonriver_runtime::Treasury,
			EthereumXcm: moonriver_runtime::EthereumXcm,
			Proxy: moonriver_runtime::Proxy,
			EVM: moonriver_runtime::EVM,
		}
	}
}

// ---------------------------------------------------------------------------
// Sibling parachain declaration (para 2005) — another Moonriver instance
// ---------------------------------------------------------------------------
decl_test_parachains! {
	pub struct SiblingPara {
		genesis = moonriver_genesis(SIBLING_PARA_ID),
		on_init = {
			crate::network::satisfy_moonriver_inherents();
		},
		runtime = moonriver_runtime,
...
/// Execute a closure on the Moonriver parachain (para 2004), automatically
/// satisfying mandatory inherent checks.
pub fn moonriver_execute_with<R>(f: impl FnOnce() -> R) -> R {
	MoonriverPara::<PolkadotMoonbeamNet>::execute_with(|| {
		satisfy_moonriver_inherents();
		f()
	})
}
...
/// Patch storage to satisfy Moonriver's mandatory inherent checks.
/// Called automatically by [`moonriver_execute_with`] / [`sibling_execute_with`].
pub(crate) fn satisfy_moonriver_inherents() {
	pallet_author_inherent::Author::<moonriver_runtime::Runtime>::put(
		moonriver_runtime::AccountId::from([1u8; 20]),
	);
	pallet_author_inherent::InherentIncluded::<moonriver_runtime::Runtime>::put(true);
...
pub fn init_network() {
	...
	MoonriverPara::<PolkadotMoonbeamNet>::ext_wrapper(|| {
		frame_support::storage::unhashed::kill(&frame_support::storage::storage_prefix(
			b"Randomness",
			b"NotFirstBlock",
		));
...
/// its price in the XCM weight trader. Call inside `moonriver_execute_with` or
...
/// Call inside `moonriver_execute_with` or `sibling_execute_with`.
pub fn set_westend_relay_indices() {
	...
	pallet_xcm_transactor::RelayIndices::<moonriver_runtime::Runtime>::put(indices);
}
...
fn moonriver_genesis(para_id: u32) -> sp_core::storage::Storage {
	use moonriver_runtime::{currency::MOVR, AccountId, Runtime};
	use sp_runtime::BuildStorage;
	...
	pallet_balances::GenesisConfig::<Runtime> {
		balances: vec![
			(AccountId::from(ALITH), MOVR * 10_000),
			(AccountId::from(BALTATHAR), MOVR * 10_000),
		],
		dev_accounts: None,
	}
	.assimilate_storage(&mut t)
	...
	// Map the NimbusId emitted by `MoonriverBlockProducer` to ALITH so
