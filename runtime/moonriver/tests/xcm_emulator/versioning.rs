...
fn xcm_version_discovery_with_relay() {
	...
	WestendRelay::<PolkadotMoonbeamNet>::execute_with(|| {
		...
		let fees = westend_runtime::Runtime::query_delivery_fees(
			xcm::VersionedLocation::from(Location::new(0, [Parachain(MOONRIVER_PARA_ID)])),
...
fn xcm_version_discovery_with_sibling() {
	...
	sibling_execute_with(|| register_glmr_foreign_asset(MOONRIVER_PARA_ID));
	...
	sibling_execute_with(|| {
		use xcm_runtime_apis::fees::runtime_decl_for_xcm_payment_api::XcmPaymentApiV1;
		let fees = moonriver_runtime::Runtime::query_delivery_fees(
			xcm::VersionedLocation::from(Location::new(1, [Parachain(MOONRIVER_PARA_ID)])),
			xcm::VersionedXcm::from(Xcm::<()>(vec![ClearOrigin])),
		);
		assert!(
			fees.is_ok(),
