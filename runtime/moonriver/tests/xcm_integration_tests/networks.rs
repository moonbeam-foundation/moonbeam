...
use crate::chains::{
	...
	moonriver::{moonriver_ext, MOONRIVER_PARA_ID},
...
thread_local! {
	...
	pub static MOONRIVER_EXT: RefCell<TestExternalities> = RefCell::new(moonriver_ext());
...
pub fn reset_networks() {
	...
	MOONRIVER_EXT.with(|ext| *ext.borrow_mut() = moonriver_ext());
...
pub fn moonriver_execute_with<R>(f: impl FnOnce() -> R) -> R {
	MOONRIVER_EXT.with(|ext| ext.borrow_mut().execute_with(f))
}
...
pub fn process_dmp_messages() {
	while let Some((para_id, msg)) = DMP_QUEUE.with(|q| q.borrow_mut().pop_front()) {
		...
		match para_id_u32 {
			id if id == MOONRIVER_PARA_ID => {
				moonriver_execute_with(|| {
					// Decode the versioned XCM and execute directly
					if let Ok(versioned) =
						xcm::VersionedXcm::<moonriver_runtime::RuntimeCall>::decode(&mut &msg[..])
					{
						if let Ok(xcm_msg) = versioned.try_into() {
							let origin = Location::parent();
							let mut hash = sp_io::hashing::blake2_256(&msg);
							let _ = XcmExecutor::<moonriver_runtime::xcm_config::XcmExecutorConfig>::prepare_and_execute(
                                origin,
                                xcm_msg,
                                &mut hash,
                                Weight::MAX,
...
pub fn moonriver_execute_and_dispatch<R>(f: impl FnOnce() -> R) -> R {
	let result = moonriver_execute_with(f);
	dispatch_xcm_buses();
	result
}
...
pub fn moonriver_sovereign_account() -> sp_runtime::AccountId32 {
	parachain_sovereign_account(MOONRIVER_PARA_ID)
}
...
pub mod para_ids {
	pub const MOONRIVER: u32 = super::MOONRIVER_PARA_ID;
