...
fn add_supported_asset(asset_type: parachain::AssetType, units_per_second: u128) -> Result<(), ()> {
	...
	let VersionedLocation::V5(location_v4) = VersionedLocation::V3(location_v3)
		.into_version(4)
		.map_err(|_| ())?
	else {
		return Err(());
...
fn send_relay_asset_to_relay() {
	...
	ParaA::execute_with(|| {
		...
		assert_ok!(PolkadotXcm::transfer_assets(
			...
			Box::new(VersionedAssets::V5(vec![asset].into())),
			0,
...
fn transact_through_sovereign() {
	...
	ParaA::execute_with(|| {
		assert_ok!(XcmTransactor::transact_through_sovereign(
			parachain::RuntimeOrigin::root(),
			Box::new(xcm::VersionedLocation::V5(dest)),
			Some(PARAALICE.into()),
			CurrencyPayment {
				currency: Currency::AsMultiLocation(Box::new(xcm::VersionedLocation::V5(
					Location::parent()
...
fn receive_asset_with_no_sufficients_not_possible_if_non_existent_account() {
	...
	Relay::execute_with(|| {
		assert_ok!(RelayChainPalletXcm::limited_reserve_transfer_assets(
			relay_chain::RuntimeOrigin::signed(RELAYALICE),
			Box::new(Parachain(1).into()),
			Box::new(VersionedLocation::V5(dest.clone()).clone().into()),
			Box::new(([] /* Here */, 123).into()),
			0,
			WeightLimit::Unlimited
		));
...
fn send_statemine_asset_from_para_a_to_statemine_with_relay_fee() {
	...
	Relay::execute_with(|| {
		assert_ok!(RelayChainPalletXcm::limited_reserve_transfer_assets(
			...
			Box::new(Parachain(1).into()),
			Box::new(
				VersionedLocation::V5(parachain_beneficiary_from_relay)
					.clone()
					.into()
			),
	...
	Statemine::execute_with(|| {
		...
		assert_ok!(StatemineChainPalletXcm::limited_reserve_transfer_assets(
			...
			Box::new(Location::new(1, [Parachain(1)]).into()),
			Box::new(
				VersionedLocation::V5(parachain_beneficiary_from_statemine)
					.clone()
					.into()
			),
			Box::new(
	...
	ParaA::execute_with(|| {
		...
		assert_ok!(PolkadotXcm::transfer_assets(
			...
			Box::new(VersionedLocation::V5(chain_part)),
			Box::new(VersionedLocation::V5(beneficiary)),
			Box::new(VersionedAssets::V5(assets_to_send.into())),
			1,
...
fn send_dot_from_moonbeam_to_statemine_via_xtokens_transfer() {
	...
	Relay::execute_with(|| {
		assert_ok!(RelayChainPalletXcm::limited_teleport_assets(
			...
			Box::new(
				VersionedLocation::V5(statemine_beneficiary_absolute)
					.clone()
					.into()
			),
			Box::new(([], 200).into()),
	...
	Statemine::execute_with(|| {
		...
		assert_ok!(StatemineChainPalletXcm::limited_reserve_transfer_assets(
			...
			Box::new(Location::new(1, [Parachain(1)]).into()),
			Box::new(
				VersionedLocation::V5(parachain_beneficiary_absolute.clone())
					.clone()
					.into()
			),
	...
	Statemine::execute_with(|| {
		...
		assert_ok!(StatemineChainPalletXcm::limited_reserve_transfer_assets(
			...
			Box::new(Location::new(1, [Parachain(1)]).into()),
			Box::new(
				VersionedLocation::V5(parachain_beneficiary_absolute)
					.clone()
					.into()
			),
...
fn send_dot_from_moonbeam_to_statemine_via_xtokens_transfer_multicurrencies() {
	...
	ParaA::execute_with(|| {
		...
		assert_ok!(PolkadotXcm::transfer_assets(
			...
			Box::new(VersionedLocation::V5(chain_part)),
			Box::new(VersionedLocation::V5(beneficiary)),
			Box::new(VersionedAssets::V5(vec![asset_1, asset_2].into())),
			1,
			WeightLimit::Limited(Weight::from_parts(80_000_000u64, 100_000u64))
		));
...
fn send_dot_from_moonbeam_to_statemine_via_xtokens_transfer_multiassets() {
	...
	ParaA::execute_with(|| {
		assert_ok!(PolkadotXcm::transfer_assets(
			...
			Box::new(VersionedLocation::V5(chain_part)),
			Box::new(VersionedLocation::V5(beneficiary)),
			Box::new(VersionedAssets::V5(assets_to_send)),
			0,
			WeightLimit::Limited(Weight::from_parts(80_000_000u64, 100_000u64))
		));
...
fn transact_through_signed_multilocation_para_to_para() {
	...
	ParaA::execute_with(|| {
		...
		assert_ok!(XcmTransactor::set_transact_info(
			parachain::RuntimeOrigin::root(),
			// ParaB
			Box::new(xcm::VersionedLocation::V5(para_b_location.clone())),
		...
		assert_ok!(XcmTransactor::set_fee_per_second(
			parachain::RuntimeOrigin::root(),
			Box::new(xcm::VersionedLocation::V5(para_b_balances.clone())),
	...
	ParaA::execute_with(|| {
		assert_ok!(XcmTransactor::transact_through_signed(
			parachain::RuntimeOrigin::signed(PARAALICE.into()),
			Box::new(xcm::VersionedLocation::V5(para_b_location)),
			CurrencyPayment {
				currency: Currency::AsMultiLocation(Box::new(xcm::VersionedLocation::V5(
					para_b_balances
				))),
