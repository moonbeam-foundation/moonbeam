...
fn transact_through_signed_multilocation_para_to_para_ethereum() {
	...
	let mut encoded: Vec<u8> = Vec::new();
	let index =
		<parachain::Runtime as frame_system::Config>::PalletInfo::index::<parachain::Ethereum>()
			.unwrap() as u8;
	...
	let eth_tx = fp_xcm::EthereumXcmTransaction::V1(fp_xcm::EthereumXcmTransactionV1 {
		gas_limit: U256::from(21000),
		fee_payment: fp_xcm::EthereumXcmFee::Auto(fp_xcm::AutoEthereumXcmFee::Low),
	...
	// Then call bytes
	let mut call_bytes = pallet_ethereum::Call::<parachain::Runtime>::transact_xcm {
	...
	ParaB::execute_with(|| {
		// Make sure the EVM transfer did not go through
