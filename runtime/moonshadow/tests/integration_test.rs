...
//! Moonshadow Runtime Integration Tests
...
fn join_collator_candidates() {
	ExtBuilder::default()
		.with_balances(vec![
			...
			(AccountId::from(BOB), 2_000 * MSHD),
			(AccountId::from(CHARLIE), 1_100 * MSHD),
			(AccountId::from(DAVE), 1_000 * MSHD),
		])
		.with_collators(vec![
			(AccountId::from(ALICE), 1_000 * MSHD),
			(AccountId::from(BOB), 1_000 * MSHD),
		])
		.with_nominations(vec![
			(AccountId::from(CHARLIE), AccountId::from(ALICE), 50 * MSHD),
			(AccountId::from(CHARLIE), AccountId::from(BOB), 50 * MSHD),
		])
		.build()
		.execute_with(|| {
			assert_noop!(
...
fn initialize_crowdloan_addresses_with_batch_and_pay() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let per_block = (1_050_000 * MSHD) / vesting_period;

			assert_eq!(
				CrowdloanRewards::accounts_payable(&AccountId::from(CHARLIE))
					.unwrap()
					.claimed_reward,
				(450_000 * MSHD) + per_block
			);
			assert_eq!(
...
fn reward_info_via_precompile() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let expected_total: U256 = (1_500_000 * MSHD).into();
			let expected_claimed: U256 = (450_000 * MSHD).into();
...
fn nominate_via_precompile() {
	ExtBuilder::default()
		.with_balances(vec![
			(AccountId::from(ALICE), 3_000 * MSHD),
			(AccountId::from(BOB), 3_000 * MSHD),
		])
		.with_collators(vec![(AccountId::from(ALICE), 1_000 * MSHD)])
		.build()
		.execute_with(|| {
...
fn leave_nominators_via_precompile() {
	ExtBuilder::default()
		.with_balances(vec![
			(AccountId::from(ALICE), 1_000 * MSHD),
			(AccountId::from(BOB), 1_000 * MSHD),
			(AccountId::from(CHARLIE), 1_500 * MSHD),
		])
		.with_collators(vec![
			(AccountId::from(ALICE), 1_000 * MSHD),
			(AccountId::from(BOB), 1_000 * MSHD),
		...
		.with_nominations(vec![
			(AccountId::from(CHARLIE), AccountId::from(ALICE), 500 * MSHD),
			(AccountId::from(CHARLIE), AccountId::from(BOB), 500 * MSHD),
		])
		.build()
		.execute_with(|| {
...
fn candidate_count_via_precompile() {
	ExtBuilder::default()
		.with_balances(vec![
			(AccountId::from(ALICE), 1_100 * MSHD),
			(AccountId::from(BOB), 1_100 * MSHD),
			(AccountId::from(CHARLIE), 1_100 * MSHD),
			(AccountId::from(DAVE), 1_100 * MSHD),
		])
		.with_collators(vec![
			(AccountId::from(ALICE), 1_000 * MSHD),
			(AccountId::from(BOB), 1_000 * MSHD),
			(AccountId::from(CHARLIE), 1_000 * MSHD),
			(AccountId::from(DAVE), 1_000 * MSHD),
		])
		.build()
		.execute_with(|| {
...
fn collator_nomination_count_via_precompile() {
	ExtBuilder::default()
		.with_balances(vec![
			(AccountId::from(ALICE), 1_100 * MSHD),
			(AccountId::from(BOB), 100 * MSHD),
			(AccountId::from(CHARLIE), 100 * MSHD),
			(AccountId::from(DAVE), 100 * MSHD),
		])
		.with_collators(vec![(AccountId::from(ALICE), 1_000 * MSHD)])
		.with_nominations(vec![
			(AccountId::from(BOB), AccountId::from(ALICE), 100 * MSHD),
			(AccountId::from(CHARLIE), AccountId::from(ALICE), 100 * MSHD),
			(AccountId::from(DAVE), AccountId::from(ALICE), 100 * MSHD),
		])
		.build()
		.execute_with(|| {
			let staking_precompile_address = H160::from_low_u64_be(2048);
...
fn nominator_nomination_count_via_precompile() {
	ExtBuilder::default()
		...
		.with_nominations(vec![
			(AccountId::from(DAVE), AccountId::from(ALICE), 100 * MSHD),
			(AccountId::from(DAVE), AccountId::from(BOB), 100 * MSHD),
			(AccountId::from(DAVE), AccountId::from(CHARLIE), 100 * MSHD),
		])
		.build()
		.execute_with(|| {
			let staking_precompile_address = H160::from_low_u64_be(2048);
