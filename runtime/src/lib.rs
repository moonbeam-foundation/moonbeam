...
fn ensure_linear_cost(
    ...
    base: usize,
    word: usize
) -> Result<usize, frame_evm::ExitError> {
    let cost = base.checked_add(
        word.checked_mul(len.saturating_add(31) / 32).ok_or(frame_evm::ExitError::OutOfGas)?
    ).ok_or(frame_evm::ExitError::OutOfGas)?;
    if let Some(target_gas) = target_gas {
        if cost > target_gas {
            return Err(frame_evm::ExitError::OutOfGas)
        }
    }
    Ok(cost)
}
...
impl frame_evm::Precompiles for MoonbeamPrecompiles {
    fn execute(
        address: H160,
        input: &[u8],
        target_gas: Option<usize>
    ) -> Option<core::result::Result<(frame_evm::ExitSucceed, Vec<u8>, usize), frame_evm::ExitError>> {
...
impl frame_evm::Trait for Runtime {
    ...
    type Currency = Balances;
    type Event = Event;
    type Precompiles = MoonbeamPrecompiles;
