...
contract Messages is Getters {
    ...
    function toBytes32String(bytes32 x) internal pure returns (string memory) {
    ...
    function toAsciiString(
        ...
        for (uint256 i = 0; i < 20; i++) {
            bytes1 b = bytes1(
                uint8(uint256(uint160(y)) / (2 ** (8 * (19 - i))))
            );
            bytes1 hi = bytes1(uint8(b) / 16);
            bytes1 lo = bytes1(uint8(b) - 16 * uint8(hi));
            s[45 + 2 * i] = char(hi);
    ...
    function NumbertoString(uint8 b) internal pure returns (string memory) {
    ...
    function verifySignatures(
        ...
        for (uint i = 0; i < signatures.length; i++) {
            ...
            require(
                signatory == guardianSet.keys[sig.guardianIndex],
                toAsciiString(signatory, guardianSet.keys[sig.guardianIndex])
            );
    ...
    event FailedSignature(address indexed expected, address indexed retrieved);
    ...
    function parseVM(
        ...
        require(vm.version == 1, NumbertoString(vm.version));
