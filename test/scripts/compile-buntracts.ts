...
async function main(args: any) {
  ...
  const contractSourcePaths = [
    ...nodeFs.readdirSync(precompilesPath).map((filename) => ({
  ...
  for (const contractPath of contractSourcePaths) {
    ...
    for (let filepath of contracts) {
      ...
      sourceByReference[ref] = await Bun.file(filepath).text();
  ...
  if (tempFileExists) {
    contractMd5 = JSON.parse(await Bun.file(tempFile).text());
    for (const contract of Object.keys(sourceToCompile)) {
      const path = filePaths.find((path) => path.includes(contract));
      const contractHash = computeHash(await Bun.file(path!).text());
  ...
  const compilePromises = contractsToCompile.map(async (ref) => {
    try {
      await compile(ref, outputDirectory);
      await Bun.write(tempFile, JSON.stringify(contractMd5, null, 2));
    } catch (e) {
...
async function compile(
  ...
  await Promise.all(
    Object.keys(compiledContracts).map(async (contractName) => {
      ...
      await Bun.write(dest, JSON.stringify(compiledContracts[contractName], null, 2));
...
async function getFiles(dir) {
  const subdirs = nodeFs.readdirSync(dir);
  const files = await Promise.all(
    subdirs.map(async (subdir) => {
      const res = path.resolve(dir, subdir);
      return nodeFs.statSync(res).isDirectory() ? getFiles(res) : res;
    })
  );
...
function computeHash(input: string): string {
  hasher.update(input + solcVersion);
