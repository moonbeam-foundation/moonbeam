...
function parameterType(context: DevModeContext, module: string, name: string, value: unknown) {
    ...
    const runtimeParameter = context
        .polkadotJs()
        .createType(`${RUNTIME}${CRATE}RuntimeParameters`, {
            [module]: paramWrapper
        });
...
function parameterKey(context: DevModeContext, module: string, name: string) {
    ...
    const keyWrapper = context
        .polkadotJs()
        .createType(`${RUNTIME}${CRATE}RuntimeParametersKey`, {
            [module]: key
        });
...
describeSuite({
    ...
    testCases: ({ it, context, log }) => {
            ...
            it({
                ...
                test: async () => {
                    ...
                    const param = parameterType(
                        context, module, name, value,
                    );

                    const res = await context.createBlock(
                        context
                            .polkadotJs()
                            .tx.parameters.setParameter(param.toU8a())
                            .signAsync(alith),
                        { allowFailures: true }
                    );
        ...
        testParam(
            "XcmExecutor",
            "XcmFeesAccount",
            ["AccountId20", hexToU8a(BALTATHAR_ADDRESS)],
        );
    },
})