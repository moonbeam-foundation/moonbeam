    ...
    it({
      ...
      test: async () => {
        ...
        const block = await context.createBlock(
          context
            .polkadotJs()
            .tx.parachainStaking.delegateWithAutoCompound(alith.address, minDelegatorStk.subn(10), 0, 0, 0, 0)
            .signAsync(ethan)
        );
    ...
    it({
      ...
      test: async () => {
        const block = await context.createBlock(
          context
            .polkadotJs()
            .tx.parachainStaking.delegateWithAutoCompound(baltathar.address, MIN_GLMR_DELEGATOR, 0, 0, 0, 0)
            .signAsync(ethan)
        );
        expect(block.result!.successful!).to.be.false;
    ...
    it({
      ...
      test: async () => {
        const block = await context.createBlock(
          context
            .polkadotJs()
            .tx.parachainStaking.delegateWithAutoCompound(ethan.address, MIN_GLMR_DELEGATOR, 0, 0, 0, 0)
            .signAsync(ethan)
        );
        expect(block.result!.successful!).to.be.false;
        expect(block.result!.error!.name).to.equal("CandidateDNE");
