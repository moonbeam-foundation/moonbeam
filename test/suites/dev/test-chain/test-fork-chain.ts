...
import { expect, describeSuite, beforeAll, beforeEach } from "@moonwall/cli";
...
import { BALTATHAR_ADDRESS, GLMR, mapExtrinsics } from "@moonwall/util";
  ...
  testCases: ({ context, it, log }) => {
    ...
    it({
      ...
      test: async function () {
        ...
        const ethHash1 = (await context.viemClient("public").getBlock({ blockNumber: 1n })).hash;
        const ethHash2 = (await context.viemClient("public").getBlock({ blockNumber: 2n })).hash;

        // Now lets fork the chain
        const currentHeight = await context.viemClient("public").getBlockNumber();
        ...
        expect(
          (await context.viemClient("public").getBlock({ blockNumber: 1n })).hash,
          "Ethereum blocks should have changed"
        ).to.not.equal(ethHash1);
        expect(
          (await context.viemClient("public").getBlock({ blockNumber: 2n })).hash,
          "Ethereum blocks should have changed"
        ).to.not.equal(ethHash2);
        expect((await context.viemClient("public").getBlock()).number).toBe(currentHeight + 1n);
      },
    });
    ...
    for (const txnType of TransactionTypes) {
      it({
        ...
        test: async function () {
          ...
          const retractedTx = await context
            .viemClient("public")
            .getTransaction({ hash: insertedTx });
          ...
          const finalTx = await context.viemClient("public").getTransaction({ hash: insertedTx });
          expect(
            finalTx.blockHash,
