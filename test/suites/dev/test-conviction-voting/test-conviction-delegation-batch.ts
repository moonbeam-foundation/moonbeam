  ...
  testCases: ({ context, it }) => {
    ...
    beforeAll(async () => {
      ...
      for (const [id, origin] of origins) {
        const proposal = context.polkadotJs().tx.identity.setIdentity({ display: { raw: `Props ${id}` } });
    ...
    it({
      ...
      test: async function () {
        ...
        const blockResult = await context.createBlock(
          ...
          context.polkadotJs().tx.utility.batchAll(
            origins.map(([id, _], i) =>
              context
                .polkadotJs()
        ...
        await Promise.all(origins.map(async ([id, _], i) => {
