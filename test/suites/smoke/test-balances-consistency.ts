      ...
      test: async function () {
        ...
        const expectedReserveByAccount: {
          ...
          assetsMetadata.map((assetMetadata) => ({
            accountId: `0x${assets
              .find(
                (asset) => asset[0].toHex().slice(-64) == assetMetadata[0].toHex().slice(-64)
              )[1]
              .unwrap()
              .owner.toHex()
              .slice(-40)}`,
          ...
          .reduce((p, v) => {
            ...
            p[v.accountId].total += Object.keys(v.reserved).reduce(
              (p, key) => p + v.reserved[key],
              0n
            );
        ...
        if (failedLocks.length > 0 || failedReserved.length > 0) {
          ...
          expect(failedLocks.length, `Failed accounts locks: ${failedLocks.join(", ")}`).to.equal(
            0
          );
        }
