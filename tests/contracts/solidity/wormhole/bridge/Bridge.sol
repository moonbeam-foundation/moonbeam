...
contract Bridge is BridgeGovernance, ReentrancyGuard {
    ...
    function updateWrapped(
        ...
        require(verifyBridgeVM(vm), "invalid emitter 1");
    ...
    function createWrapped(
        ...
        require(valid, reason);
        require(verifyBridgeVM(vm), "invalid emitter 2");
    ...
    function _completeTransfer(
        ...
        require(valid, reason);
        require(verifyBridgeVM(vm), "invalid emitter 3");
    ...
    ) public pure returns (uint8 payloadID) {
    ...
    function parseAssetMeta(
        bytes memory encoded
    ) public pure returns (BridgeStructs.AssetMeta memory meta) {
        uint index = 0;
    ...
    function parseTransfer(
        bytes memory encoded
    ) public pure returns (BridgeStructs.Transfer memory transfer) {
        uint index = 0;
    ...
    function parseTransferWithPayload(
        bytes memory encoded
    ) public pure returns (BridgeStructs.TransferWithPayload memory transfer) {
        uint index = 0;
