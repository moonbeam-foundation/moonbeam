...
const BLOCK_TO_WAIT = (process.env.BLOCK_TO_WAIT && parseInt(process.env.BLOCK_TO_WAIT)) || 5;
...
describeParachain(
  ...
  (context) => {
    it("should not fail", async function () {
      this.timeout((3600 + BLOCK_TO_WAIT * 12) * 1000);
      ...
      process.stdout.write(`Waiting extra ${BLOCK_TO_WAIT} block being produced...`);
      await context.waitBlocks(BLOCK_TO_WAIT); // Make sure the new runtime is producing blocks
