...
describeParachain(
  ...
  (context) => {
    let keyring: Keyring,
      ...
      assetId: string;
    before(
      "First send relay chain asset to parachain A and B, and register transactors",
      async function () {
        keyring = new Keyring({ type: "sr25519" });
        ...
        // Fees
        const feeParaA = (await reserveTrasnsferAssetsCallParaA.paymentInfo(aliceRelay))
          .partialFee as any;
        ...
        await createBlockWithExtrinsicParachain(
          relayOne,
          aliceRelay,
          reserveTrasnsferAssetsCallParaA
        );
        ...
        // Fees
        const feeParaB = (await reserveTrasnsferAssetsCallParaB.paymentInfo(aliceRelay))
          .partialFee as any;

        let expectedAfterRelayBalance =
          BigInt(beforeAliceRelayBalance) -
          BigInt(feeParaA) -
          BigInt(feeParaB) -
          BigInt(THOUSAND_UNITS) -
          BigInt(THOUSAND_UNITS);
        ...
        await createBlockWithExtrinsicParachain(
          relayOne,
          aliceRelay,
          reserveTrasnsferAssetsCallParaB
        );
    ...
    it.only("should be able to receive a non-reserve asset in para b from para a", async function () {
      ...
      const encodedCall = relayOne.tx.registrar.reserve().method.toHex() || "";
      ...
      // Assert the second message never got executed

      expect(afterAliceRelayBalance.eq(beforeAliceRelayBalance)).to.eq(true);
    });
  }
);
