...
describeSmokeSuite("S1300", `Verify local asset count, assetIds and deposits`, (context) => {
  ...
  it(`should match asset deposit entries with number of assets` + ` #C100`, async function () {
    expect(
  ...
  it(
    `should ensure localAssetCounter is higher than number of local assets` + ` #C200`,
    async function () {
      expect(
        localAssetCounter,
  ...
  it(
    `assetIds stored by assetManager are also created in LocalAssets` + ` #C300`,
    async function () {
