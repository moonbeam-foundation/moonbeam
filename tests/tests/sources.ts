export const contractSources: { [key: string]: string } = {
  ...
  TestCallList: `
    ...
    contract affectedContract {
        address public stealContract;
    
        // Provide the stealContract Address
        constructor (address _stealContract) {
            stealContract = _stealContract;
        }
    ...
    contract Steal {
        address public asset = 0x0000000000000000000000000000000000000802;
        address public beneficiary ;
    
        event isSuccess(bool, bool);
    
        constructor() {
            beneficiary = msg.sender;
        }
    
            // Function that will steal funds from affectedContract using the precompile
            // it wont be able to steal funds from the WETH contract 
            function stealNow(address _erc20Address) public {
            uint256 balance = IERC20(asset).balanceOf(msg.sender);
            (bool success1, ) = asset.delegatecall(
                abi.encodeWithSignature("transfer(address,uint256)", beneficiary, balance));
            (bool success2, ) = _erc20Address.delegatecall(
                abi.encodeWithSignature("transfer(address,uint256)", beneficiary, balance));
