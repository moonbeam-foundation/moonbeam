...
import { GENESIS_ACCOUNT, GENESIS_ACCOUNT_BALANCE, GENESIS_ACCOUNT_PRIVATE_KEY } from "../util/constants";
...
describeDevMoonbeam("Reducible Balance", (context) => {
  ...
  it("should show the reducible balanced when some amount is locked", async function () {
    ...
    await context.polkadotApi.tx.democracy.notePreimage(encodedProposal).signAndSend(genesisAccount);

    await context.createBlock();

    // Record balance
    let beforeBalance = await context.web3.eth.getBalance(GENESIS_ACCOUNT, 1);
    ...
    expect(await context.web3.eth.getBalance(GENESIS_ACCOUNT, 1)).to.equal(
      (BigInt(beforeBalance) - BigInt(lock_amount) + BigInt(existentialDeposit)).toString()
    );
  });
});
