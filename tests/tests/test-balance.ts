...
describeWithMoonbeam("Moonbeam RPC (Balance)", `simple-specs.json`, (context) => {
  describe("Check Genesis Balance", async () => {
      ...
      signedBlock.block.extrinsics.forEach(({ method: { method, section } }, index) => {
        ...
        switch (index) {
          ...
          case 2:
            expect(
              events.length === 1 &&
                context.polkadotApi.events.system.ExtrinsicSuccess.is(events[0])
            ).to.be.true;
            break;
  ...
  describe("Check post polkadot API transfer balances", async () => {
