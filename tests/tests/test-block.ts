...
describeWithMoonbeam("Moonbeam RPC (Genesis Block)", `simple-specs.json`, (context) => {
  ...
  describe("Post Genesis Block 1", async () => {
    before(async () => {
      await createAndFinalizeBlock(context.polkadotApi, await context.polkadotApi.rpc.chain.getBlockHash(0), false);
  ...
  describe("Post Genesis Block 2 Finalization", async () => {
    before(async () => {
      await createAndFinalizeBlock(context.polkadotApi, await context.polkadotApi.rpc.chain.getBlockHash(0));
      await createAndFinalizeBlock(context.polkadotApi, await context.polkadotApi.rpc.chain.getBlockHash(1));
      let block1 = await context.web3.eth.getBlock(1);
      console.log("FIRST")
