...
describeDevMoonbeam("Contract creation (EIP2930)", (context) => {
  it("should return the transaction hash", async () => {
    const { rawTx } = await createContract(context, "TestContract", { accessList: [] });
    const { txResults } = await context.createBlock({ transactions: [rawTx] });
...
describeDevMoonbeam("Contract creation (EIP1559)", (context) => {
  it("should return the transaction hash", async () => {
    const { rawTx } = await createContract(context, "TestContract", { maxFeePerGas: 1_000_000_000 });
    const { txResults } = await context.createBlock({ transactions: [rawTx] });
...
describeDevMoonbeam("Contract creation (EIP2930)", (context) => {
  it("should not contain contract at genesis", async function () {
    const { contract, rawTx } = await createContract(context, "TestContract", { accessList: [] });
    expect(await context.web3.eth.getCode(contract.options.address)).to.deep.equal("0x");
  });
...
describeDevMoonbeam("Contract creation (EIP1559)", (context) => {
  it("should not contain contract at genesis", async function () {
    const { contract, rawTx } = await createContract(context, "TestContract", { maxFeePerGas: 1_000_000_000 });
    expect(await context.web3.eth.getCode(contract.options.address)).to.deep.equal("0x");
  });
