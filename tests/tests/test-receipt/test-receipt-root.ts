...
describeDevMoonbeam("Receipt root - With events", (context) => {
  before("Setup: Create block with multiple transaction types", async () => {
    ...
    let legacyTransaction_1 = (await createContract(context, "EventEmitter", {
      from: alith.address,
      nonce: 0,
    })).rawTx;
    ...
    let EIP2930Transaction_1 = (await createContract(context, "EventEmitter", {
      from: alith.address,
      nonce: 2,
    })).rawTx;
    ...
    let EIP1559Transaction_1 = (await createContract(context, "EventEmitter", {
      from: alith.address,
      nonce: 4,
    })).rawTx;
