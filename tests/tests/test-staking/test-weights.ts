...
describeBenchmark("Staking - Max Transaction Fit", "scheduleLeaveDelegators", async (context) => {
  ...
  await expectOk(
    context.createBlock(
      randomAccounts.map((account) =>
        context.polkadotApi.tx.parachainStaking.scheduleLeaveDelegators().signAsync(account)
      )
    )
  );
});
...
describeBenchmark("Staking - Max Transaction Fit", "executeLeaveDelegators", async (context) => {
  ...
  await expectOk(
    context.createBlock(
      randomAccounts.map((account) =>
        context.polkadotApi.tx.parachainStaking
          .executeLeaveDelegators(account.address, 1)
          .signAsync(account)
      )
    )
  );
...
async function countExtrinsics(context: DevTestContext, method: string): Promise<number> {
  const block = await context.polkadotApi.rpc.chain.getBlock();
  return block.block.extrinsics.reduce(
    (acc, ext) =>
