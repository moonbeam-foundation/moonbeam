...
import { MOONBEAM_SUPPLY_FACTOR } from "../../util/constants";
...
describeDevMoonbeamAllRuntimes(
  ...
  (context) => {
    ...
    it("should receive transact and should be able to execute", async function () {
      ...
      let targetXcmFee = BigInt(await getSupplyFactor(context)) * targetXcmWeight * 50_000n;
...
describeDevMoonbeamAllRuntimes(
  "Mock XCM - receive horizontal transact ETHEREUM (call)",
  (context) => {
    let transferredBalance;
    let sendingAddress;
    let random: KeyringPair;
