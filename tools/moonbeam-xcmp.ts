...
async function test() {
  ...
  // Check that relay chain persists storage changes (registers the necessary channel)
  ...
  // transfer_native_to_account20_parachain
  ...
  await new Promise<void>(async (resolve) => {
    const unsub2 = await moonbeam200.tx.xtransfer
      .transferNativeToAccount20Parachain(recipient, GERALD, 100000)
